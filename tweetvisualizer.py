...
df3 = (values[values > 2]/ values.sum()) * 100
...
lists = dict((k,v) for k,v in counting_hashtags.items() if v>= 15)
print(lists)
SMALL_SIZE = 11
keys = lists.keys()
values = lists.values()
...
sorted_count.drop(['tweetid','created_at','user_name','text','favourite_count','retweet_count','location','hashtags','Time','Counter'], axis=1,inplace=True)
plt.hist(sorted_count,cumulative=True, label= 'CDF',histtype='step',alpha=0.8)
plt.show()
...
df1['Time'] = pd.to_datetime(df1['created_at']).dt.hour
df1['Counter'] = 1
time_series = df1.groupby('Time')['Counter'].sum().reset_index()
plt.bar(time_series['Time'], time_series['Counter'])
